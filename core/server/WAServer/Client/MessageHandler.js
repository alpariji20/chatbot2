...
class Message {
    ...
    async mainHandler() {
        ...
        try {
            ...
            // Mengirim pesan ke main.py dan mendapatkan respons
            const response = await axios.post('http://localhost:5570/chat', { user_input: body });
            const replyMessage = response.data.msg;

            if (replyMessage) {
                await ilsya.sendText(replyMessage, m);
            }
